...
const ArtistUpdated = () => {
  ...
  const handleAddLocation = async () => {
    if (newLocation.trim()) {
      const formattedLocation = newLocation.trim() + ",,"; // Ensures consistent format
      ...
      try {
        ...
        // Add a fake date along with the new location
        const fakeDate = "fake"; // Example of a fake date
        await axios.post(
          `https://spotifyapp-backend.onrender.com/add-date/${selectedArtist._id}`,
          { date: fakeDate, location: formattedLocation }
        );
        console.log("Fake date added:", fakeDate);

        // Update selectedArtist state with both new location and fake date
        setSelectedArtist((prev) => ({
          ...prev,
          locations: [...prev.locations, formattedLocation],
          dates: [...prev.dates, fakeDate], // Adds fake date to dates array
        }));
        setNewLocation("");
        setView("date");
      } catch (error) {
        console.error("Error adding location or date:", error);
      }
    } else {
  ...
  const handleAddDate = async () => {
    if (newDate.trim() && selectedLocation) {
      ...
      try {
        ...
        // If the location exists, add both location and date each time
        if (locationIndex !== -1) {
          ...
          // If the location does not exist, add both new location and date
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            >
              {selectedArtist?.locations?.length > 0 ? (
                selectedArtist.locations
                  .map((loc, locIndex) => {
                    // Check if there is no "fake" date at this specific index
                    const dateAtIndex = selectedArtist.dates[locIndex];

                    // If the date is "fake," return null to exclude this entry; otherwise, return the location element
                    if (dateAtIndex === "fake") return null;
                    ...
                    <div className="overflow-y-auto h-[600px] w-[350px] mt-[2px] hide-scrollbar">
                      {datesForSelectedLocation.length > 0 ? (
                        datesForSelectedLocation
                          .filter((date) => date.toLowerCase() !== "fake")
                          .map((date, index) => (
                            <div
                              key={index}
